...
describe("withEntry", () => {
	it("should add new entry to entries", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_entry/addNewEntry");
	...
	it("should generate block name when empty", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_entry/addEmptyBlockName");
	...
	it("should maintain existing entries when adding to a list", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_entry/addEntryToList");
...
describe("withSubEntry", () => {
	it("adding first entry should convert to group", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_sub_entry/addConvertToGroup");
	...
	it("adding first entry for folder should populate subentries", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_sub_entry/addFolderPopulateSubEntries");
		const output = withSubEntry(input, "New Entry", currentTime);
		expect(stripEntryRuntimeData(output)).toEqual(stripEntryRuntimeData(expected));
	});
	...
	it("adding first entry for folder should extend subentries", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_sub_entry/addFolderExtendSubEntries");
	...
	it("adding to group should extend sub entries", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_sub_entry/addToGroupExtendSubEntries");
	...
	it("empty name should generate a part name (single)", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_sub_entry/emptyNameCreatePartNameSingle");
	...
	it("empty name should generate a part name (group)", async () => {
		const { input, currentTime, expected } =
			await import("@fixtures/manipulating/adding_sub_entry/emptyNameCreatePartNameGroup");
